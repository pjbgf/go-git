...
type LazyObjectWriter interface {
	LazyWriter(typ plumbing.ObjectType, size int64) (w io.WriteCloser, err error)
}
	...
	for i := uint32(0); i < p.count; i++ {
		...
		if p.storage != nil && !delta {
			...
			if low, ok := p.storage.(LazyObjectWriter); ok {
				ow, err := low.LazyWriter(oh.Type, oh.Length)
				if err != nil {
					return err
				}
...
func (p *Parser) resolveObject(
	...
	if p.storage != nil {
		if low, ok := p.storage.(LazyObjectWriter); ok {
			...
			ow, err := low.LazyWriter(typ, o.Size())
			if err != nil {
				return err
			}
	...
	err = applyPatchBase(o, base, buf, mw)
	if err != nil {
		return err
	}
	...
	if obj != nil {
		obj.SetType(o.Type)
		obj.SetSize(o.Size()) // Size here is correct as it was populated by patch above.
		if _, err := p.storage.SetEncodedObject(obj); err != nil {
			return err
		}
	}
...
func applyPatchBase(ota *objectInfo, base io.ReaderAt, delta io.Reader, target io.Writer) error {
	if target == nil {
	...
	sz, h, err := patchDeltaWriter(target, base, delta, typ)
	if err != nil {
		return err
	}
