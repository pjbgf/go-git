...
type MemoryIndex struct {
	...
	offsetHash       map[int64]hasher.ImmutableHash
...
func NewMemoryIndex(f format.ObjectFormat) *MemoryIndex {
	idx := &MemoryIndex{
		objectFormat: f,
	}
	...
	idx.offsetHash = make(map[int64]hasher.ImmutableHash)

	return idx
}

func (idx *MemoryIndex) reset(nameLength int, objectFormat format.ObjectFormat) {
	// Clear out fanout.
	...
	idx.nameLength = nameLength
	idx.objectFormat = objectFormat
}

func (idx *MemoryIndex) findHashIndex(h hasher.ImmutableHash) (int, bool) {
	...
	for {
		mid := (low + high) >> 1
		objectIDLength := uint64(idx.nameLength)
...
func (idx *MemoryIndex) Contains(h hasher.ImmutableHash) (bool, error) {
	_, ok := idx.findHashIndex(h)
	return ok, nil
}

// FindOffset implements the Index interface.
func (idx *MemoryIndex) FindOffset(h hasher.ImmutableHash) (int64, error) {
...
func (idx *MemoryIndex) FindCRC32(h hasher.ImmutableHash) (uint32, error) {
	k := idx.FanoutMapping[h.Sum()[0]]
	i, ok := idx.findHashIndex(h)
	if !ok {
		return 0, plumbing.ErrObjectNotFound
...
func (idx *MemoryIndex) FindHash(o int64) (hasher.ImmutableHash, error) {
	var hash hasher.ImmutableHash
	var ok bool
	...
	if !idx.offsetHashIsFull || idx.offsetHash == nil {
		if err := idx.genOffsetHash(); err != nil {
			return hasher.ZeroFromObjectFormat(idx.objectFormat), err
		}
	...
	if !ok {
		return hasher.ZeroFromObjectFormat(idx.objectFormat), plumbing.ErrObjectNotFound
	}
...
func (idx *MemoryIndex) genOffsetHash() error {
	...
	idx.offsetHash = make(map[int64]hasher.ImmutableHash, count)
	...
	for firstLevel, fanoutValue := range idx.Fanout {
		...
		for secondLevel := uint32(0); i < fanoutValue; i++ {
			objectIDLength := uint32(idx.nameLength)
			copy(hash.Sum(), idx.Names[mappedFirstLevel][secondLevel*objectIDLength:])
...
func (i *idxfileEntryIter) Next() (*Entry, error) {
	for {
		...
		h, _ := hasher.FromBytes(i.idx.Names[mappedFirstLevel][i.secondLevel*i.idx.nameLength:])
		entry := &Entry{
			Hash:   h,
